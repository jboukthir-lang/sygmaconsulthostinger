...
CREATE POLICY "Public can read service appointment types"
    ON service_appointment_types FOR SELECT
    USING (true);

-- Allow authenticated to modify
CREATE POLICY "Authenticated can modify service appointment types"
    ON service_appointment_types FOR ALL
...
-- 7. Grant permissions
GRANT SELECT ON service_appointment_types TO anon, authenticated;
...
BEGIN
    ...
    SELECT COUNT(*) INTO appointment_types_count FROM appointment_types;

    RAISE NOTICE '✅ Services table enhanced successfully!';
    RAISE NOTICE '';
    RAISE NOTICE '📊 Statistics:';
    RAISE NOTICE '   - Total Services: %', services_count;
    RAISE NOTICE '   - Total Appointment Types: %', appointment_types_count;
    RAISE NOTICE '';
